<?php get_header(); ?>
	...
	<div class="sc"><?php corner_start("");?>
		...
		<h2><?php _e('Archive'); the_author_nickname();?></h2>
		
		<?php } elseif (is_tag()) { ?>
		<h2><?php _e('Tag Archive For "'); echo single_tag_title().'"';?></h2>
		<?php } ?>
	...
	<?php corner_end();?></div>
		...
		<?php while (have_posts()) : the_post(); ?>
			...
			<div class="sc"><?php corner_start("post");?>
				...
				<p class="postmeta"> 
				<span class="red author"><?php the_author() ?></span>, <?php the_time('F j, Y') ?> @ <?php the_time() ?> 
